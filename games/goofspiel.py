...
def build_goofspiel_tree(num_players, rank):
    ...
    for hand in hands:
        ...
        build_goofspiel_hand_tree(hand, [list(range(1, rank+1)) for p in range(num_players)],
                                  [[] for p in range(num_players)], 0, 0, n, tree)
...
def build_goofspiel_hand_tree(hand, remaining_cards, played_cards, current_round, current_player, current_node, tree):
    ...
    if(len(remaining_cards[current_player]) == 0):
        tree.addLeaf(parent = current_node, utility = goofspiel_utility(hand, played_cards))
        return
    ...
    for card in current_player_cards:
        actionName = "p" + str(current_player) + "c" + str(card)
        ...
        build_goofspiel_hand_tree(hand, remaining_cards, played_cards, next_round, next_player, n, tree)
